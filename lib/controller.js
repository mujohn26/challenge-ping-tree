...
const updateTargetById = (req, res, id) => {
  try {
    client.get('target', (err, obj) => {
      if (!obj || err) {
        res.writeHead(404, { 'Content-Type': 'application/json' })
        res.end(
          JSON.stringify({
            error: 'No targets in the system'
          })
        )
      } else {
        const isIdEqual = (JSON.parse(obj).id === id)
        if (isIdEqual) {
          const targetData = JSON.parse(obj)

          const object = {
            id: req.body.id === undefined ? targetData.id : req.body.id,
            url: req.body.url === undefined ? targetData.url : req.body.url,
            value: req.body.value === undefined ? targetData.value : req.body.value,
            maxAcceptsPerDay: req.body.maxAcceptsPerDay === undefined ? targetData.maxAcceptsPerDay : req.body.maxAcceptsPerDay,
            accept: req.body.accept === undefined ? targetData.accept : req.body.accept
          }
          client.set(
            'target',
            ...
            (err, reply) => {
              if (err) {
                ...
                res.end(
                  JSON.stringify({
                    error: 'Target was updated successfully'
                  })
                )
              }
            })
  ...
  try {
    client.get('target', (err, obj) => {
      if (!obj || err) {
        ...
        res.end(
          JSON.stringify({
            error: 'Target with the request  does not exist'
          })
        )
      } else {
...
module.exports = { createTarget, getAllTargets, getAllTargetById, makeRequest, updateTargetById }
